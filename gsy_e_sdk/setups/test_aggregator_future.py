...
class FutureAggregator(TestAggregatorBase):
    ...
    def _initialize_future_markets_info(self, duration_hours, market_slot_length):
        ...
        for market in range(1, number_of_open_future_markets + 1):
            slot = self.initial_slot.add(minutes=market * slot_length).format(DATE_TIME_FORMAT)
    ...
    def _recalculate_open_future_slots_and_send_forecast(self, market_info):
        if self.initial_slot is None:
            self.initial_slot = from_format(market_info["market_slot"], DATE_TIME_FORMAT) \
                .add(minutes=-slot_length)
            self._initialize_future_markets_info(future_markets_duration_hours, slot_length)
        ...
        if current_slot in self.open_future_market_slots:
            self.open_future_market_slots.remove(current_slot)
        self.open_future_market_slots.append(next_future_slot)
        print(next_future_slot, self.initial_slot.add(hours=sim_duration_hours))

        if next_future_slot == self.initial_slot.add(hours=sim_duration_hours).format(DATE_TIME_FORMAT):
            self.open_future_market_slots.pop(-1)
            return
        ...
        try:
            ...
            for area_uuid, area_dict in self.latest_grid_tree_flat.items():
                ...
                if self._can_place_forecasts(area_dict):
                    logging.warning(f"Posting ORDERS from {area_dict['area_name']} asset in the FUTURE_MARKETS")
                    for future_slot in self.open_future_market_slots:
                        ...
                        if transactions:
                            # self._test_future_posts(transactions)
                            self._has_tested_future_posts = True
    ...
    def on_finish(self, finish_info):
        if not self._has_tested_future_posts:
            error_message = (
                "FUTURE MARKET connection has not been tested successfully.")
            logging.error(error_message)
            self.errors.append(error_message)
