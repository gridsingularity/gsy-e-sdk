...
class TestRedisClientBase:
    """Tests for the redis client base behaviour."""
    ...
    def test_check_buffer_message_matching_command_and_id(self, redis_client_auto_register):
        ...
        assert redis_client_auto_register \
                   ._check_buffer_message_matching_command_and_id(message["data"]) is None
    ...
    def test_check_buffer_message_matching_command_and_id_throws_another_exception(self, redis_client_auto_register):
        ...
        with pytest.raises(RedisAPIException,
            ...
            redis_client_auto_register \
                ._check_buffer_message_matching_command_and_id(message["data"])
    ...
    def test_register(self, redis_client_auto_register):
        ...
        redis_client_auto_register.redis_db.publish \
            .assert_called_once_with(f"{AREA_ID}/register_participant", json.dumps(data))
    ...
    def test_unregister(self, redis_client_auto_register):
        ...
        self.redis_client.redis_db.publish \
            .assert_called_once_with(f"{AREA_ID}/unregister_participant",
                                     json.dumps(data))

    def test_unregister_blocking_is_active_false_throws_exception(self, redis_client_auto_register):
        """checks if is active set to false throws exception"""
        with pytest.raises(RedisAPIException,
    ...
    def test_on_event_or_response(self, redis_client_auto_register):
        """assigning the correct arguments to blocking command response and
        checking by calling on_event_or_response function with correct message"""
        message = {"data": {"device_uuid": TRANSACTION_ID,
                            "transaction_id": TRANSACTION_ID}}
        message["data"] = json.dumps(message["data"])
        redis_client_auto_register._on_event_or_response(message)
    ...
    def test_select_aggregator_self_area_uuid_none_throws_exception(self, redis_client_auto_register):
        """checks if select aggregator with area_uuid is none throws exception"""
        aggregator_uuid = str(uuid.uuid4())
        with pytest.raises(RedisAPIException,
                           match="The device/market has not ben registered yet,"
                                 " can not select an aggregator"):
            redis_client_auto_register\
                .select_aggregator(aggregator_uuid=aggregator_uuid, is_blocking=True)

    def test_select_aggregator_self_area_uuid_not_none_throws_exception(self, redis_client_auto_register):
        """checks if select aggregator with area_uuid is not none throws exception"""
        aggregator_uuid = str(uuid.uuid4())
        with pytest.raises(RedisAPIException,
    ...
    def test_unselect_aggregator_throws_exception(self, redis_client_auto_register):
        """checks if unselect aggregator throws exception"""
        aggregator_uuid = str(uuid.uuid4())
        with pytest.raises(NotImplementedError,
                           match="unselect_aggregator hasn\'t been implemented yet."):
