        ...
        for device_event in market_info["content"]:
            ...
            if key_in_dict_and_not_none(device_event, "area_uuid"):
                ...
                if "energy_requirement_kWh" in device_event["device_info"] and \
                    ...
                    rate = self.load_strategy[0]
                    energy = device_event["device_info"]["energy_requirement_kWh"]
                    self.add_to_batch_commands.bid_energy_rate(area_uuid=device_event["area_uuid"], rate=rate, energy=energy)
                ...
                if "available_energy_kWh" in device_event["device_info"] and \
                    ...
                    rate = self.pv_strategy[0]
                    energy = device_event["device_info"]["available_energy_kWh"]
                    self.add_to_batch_commands.offer_energy_rate(area_uuid=device_event["area_uuid"], rate=rate, energy=energy)
                ...
                if "energy_to_buy" in device_event["device_info"]:
                    buy_energy = device_event["device_info"]["energy_to_buy"] + device_event["device_info"]["offered_buy_kWh"]
                    sell_energy = device_event["device_info"]["energy_to_sell"] + device_event["device_info"]["offered_sell_kWh"]
                    # Battery buy strategy
                    if buy_energy > 0.0:
                        buy_rate = self.batt_buy_strategy[0]
                        self.add_to_batch_commands.bid_energy_rate(area_uuid=device_event["area_uuid"], rate=buy_rate, energy=buy_energy)
                        #print(f'{self.device_uuid_map[uuid]} BID {round(buy_energy, 4)} kWh 'f'at {round(buy_price / buy_energy, 2)}/kWh')
                    # Battery sell strategy
                    if sell_energy > 0.0:
                        sell_rate = self.batt_sell_strategy[0]
                        self.add_to_batch_commands.offer_energy_rate(area_uuid=device_event["area_uuid"], rate=sell_rate, energy=sell_energy)
        ...
        for device_event in tick_info["content"]:
            ...
            if key_in_dict_and_not_none(device_event, "area_uuid"):
                ...
                if "energy_to_buy" in device_event["device_info"]:
                    ...
                    # Battery buy strategy
                    if buy_energy > 0.0:
                        buy_rate = self.batt_buy_strategy[i]
                        self.add_to_batch_commands.bid_energy_rate(area_uuid=device_event["area_uuid"], rate=buy_rate, energy=buy_energy)
                        #print(f'{self.device_uuid_map[uuid]} BID {round(buy_energy, 4)} kWh 'f'at {round(buy_price / buy_energy, 2)}/kWh')
                    # Battery sell strategy
                    if sell_energy > 0.0:
                        sell_rate = self.batt_sell_strategy[i]
                        self.add_to_batch_commands.offer_energy_rate(area_uuid=device_event["area_uuid"], rate=sell_rate, energy=sell_energy)
                        #print(f'{self.device_uuid_map[uuid]} OFFER {round(sell_energy, 4)} kWh 'f'at {round(sell_price / sell_energy, 2)}/kWh')
