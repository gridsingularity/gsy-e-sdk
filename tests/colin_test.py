...
#domain_name = 'https://d3aweb-staging.gridsingularity.com' # leave as is
...
#websocket_domain_name = 'wss://d3aweb-staging.gridsingularity.com/external-ws' # leave as is
# Designate devices and markets to manage
# TODO update each of the below according to your team's device assignments
oracle_name = 'aggregator_chris'
load_names_1 = ['Load']
pv_names = []
storage_names = []

print(len(load_names_1))
# print(len(pv_names))
...
class Oracle(Aggregator_type):
    ...
    def on_market_cycle(self, market_info):
        ...
        for i in range(0, ticks):
            ...
            #PV->Load
            if i == 1:
                self.load_strategy.append(15)
                self.pv_strategy.append(14.8)
                self.batt_buy_strategy.append(0)
                self.batt_sell_strategy.append(35)
            ...
            #Batt->load AND PV->batt
            if i == 3:
                self.load_strategy.append(15)
                self.pv_strategy.append(14.8)
                self.batt_buy_strategy.append(14.85)
                self.batt_sell_strategy.append(14.9)
            if i == 4:
                self.load_strategy.append(15)
                self.pv_strategy.append(14.8)
                self.batt_buy_strategy.append(14.85)
            ...
            #reset
            if i == 5:
                self.load_strategy.append(0)
                self.pv_strategy.append(35)
                self.batt_buy_strategy.append(0)
                self.batt_sell_strategy.append(35)
            if i == 6:
                self.load_strategy.append(0)
...
if RUN_ON_D3A_WEB:
    ...
    # print(f"AGGR: {aggr.pubsub}")
    # from redis import StrictRedis
    # redis_db = StrictRedis.from_url('redis://localhost:6379')
    # pubsub = redis_db.pubsub()
    # pubsub.subscribe(**{'random_channel': _dummy_call})
    #
...
def register_device_list(device_names, device_args, device_uuid_map):
    for d in device_names:
        ...
        print(f"BEFORE: {aggr.device_uuid_list}")
        device.unselect_aggregator(aggr.aggregator_uuid)
        device.select_aggregator(aggr.aggregator_uuid)
        print(f"AFTER: {aggr.device_uuid_list}")
...
#print(aggr.__dict__)
device_uuid_map = {}
device_uuid_map = register_device_list(load_names_1, device_args, device_uuid_map)
